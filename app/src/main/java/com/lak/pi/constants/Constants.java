...
public interface Constants {
    ...
    int VIDEO_FILE_MAX_SIZE = 50*1024*1024; //Max size for video file in bytes | For example 5mb = 5*1024*1024

    String WEB_SITE = "http://lakpi.com/";  //web site url address
    ...
    String BILLING_KEY = "";

    // Client ID For identify the application | Must be the same with CLIENT_ID from server config: db.inc.php

    String CLIENT_ID = "158555";                         // Correct example: 12567 | Incorrect example: 0987

    // Client Secret | Text constant | Must be the same with CLIENT_SECRET from server config: db.inc.php

    String CLIENT_SECRET = "hAt1*SAoNN_siNcA58r5k5k=";    // Example: "wFt4*KBoNN_kjSdG13m1k3k="

    String API_DOMAIN = "http://lakpi.com/";  // url address to which the application sends requests | with back slash "/" at the end | example: https://mysite.com/ | for emulator on localhost: http://10.0.2.2/
    ...
    String METHOD_VIDEO_CALL = API_DOMAIN + "api/" + API_VERSION + "/method/videocall" + API_FILE_EXTENSION;
    ...
    String METHOD_GET_COINS = API_DOMAIN + "api/" + API_VERSION + "/method/coins.get" + API_FILE_EXTENSION;
    String METHOD_EXCHANGE_COINS = API_DOMAIN + "api/" + API_VERSION + "/method/coins.exchange" + API_FILE_EXTENSION;
    ...
    int GCM_NOTIFY_CALLING = 200;

    int GCM_NOTIFY_RINGING = 201;

    int GCM_NOTIFY_DISCONNECTING = 202;

    int GCM_NOTIFY_MISSEDCALL = 203;

    int GCM_NOTIFY_ACCEPT_CALL = 204;
