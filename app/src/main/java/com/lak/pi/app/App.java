...
import com.lak.pi.model.Feeling;
import com.lak.pi.model.Profile;
import com.lak.pi.model.Settings;
import com.lak.pi.util.CustomRequest;
import com.lak.pi.util.LruBitmapCache;
...
//                                            Toast.makeText(getApplicationContext(), response.toString(), Toast.LENGTH_SHORT).show();
    ...
    public void logout() { // this is called on logout
            ...
            CustomRequest jsonReq = new CustomRequest(Request.Method.POST, METHOD_ACCOUNT_LOGOUT, null,
                    new Response.Listener<JSONObject>() {
                        ...
                        public void onResponse(JSONObject response) {
                            ...
                            try {
                                Prefs.clearLocalStorage(App.getInstance());
                                if (!response.getBoolean("error")) {
                    ...
                    new Response.Listener<JSONObject>() {
                        ...
                        public void onResponse(JSONObject response) {
                            ...
                            try {
                                ...
                                System.out.println("GET_____TEST________1____"+response.has("admob"));
                                if (!response.getBoolean("error")) {
                                    ...
                                    if (response.has("free_messages_count")) { // not here
                                        Log.e("Upgrade 5644747667", ""+Prefs.getIntPref(App.getInstance(),Prefs.GENDER_KEY,-1));
                                        if(Prefs.getIntPref(getInstance(),Prefs.GENDER_KEY,-1)== 1) // 1 for female
                                        {
                                            App.getInstance().setFreeMessagesCount(250);
                                        }
                                        else
                                        {
                                    ...
                                    }if (response.has("agoraApiKey")) {

                                        App.getInstance().setAgoraApiKey(response.getString("agoraApiKey"));
                                    }
                                }
    ...
    public Boolean authorize(JSONObject authObj) {
        ...
        try {
            ...
            if (accountArray.length() > 0) {
                ...
                if (accountObj.has("free_messages_count")) { // here to set only
                    Log.e("Upgrade 12345677", ""+Prefs.getIntPref(App.getInstance(),Prefs.GENDER_KEY,-1));
                    if(Prefs.getIntPref(getInstance(),Prefs.GENDER_KEY,-1)== 1) // 1 for female
                    {
                        this.setFreeMessagesCount(250);
                    }
                    else
                    {
                ...
                System.out.println("GET_____TEST________2____"+accountObj.getInt("admob"));
...
//                                    Toast.makeText(getApplicationContext(), response.toString(), Toast.LENGTH_SHORT).show();
    ...
    public void setFreeMessagesCount(int freeMessagesCount) {
        Log.e("Upgrade 67577890", ""+Prefs.getIntPref(App.getInstance(),Prefs.GENDER_KEY,-1)+" "+freeMessagesCount);
    ...
    public void readData() {
        System.out.println("GET_____TEST________3____"+sharedPref.getInt(getString(R.string.settings_account_admob), 1));
        ...
        this.setAgoraApiKey(sharedPref.getString(getString(R.string.settings_account_agora_api_key), getAgoraApiKey()));
    ...
    public void saveData() {
        ...
        sharedPref.edit().putString(getString(R.string.settings_account_agora_api_key), this.getAgoraApiKey()).apply();
    ...
    public void setAgoraApiKey(String agoraApiKey) {
        sharedPref.edit().putString(getString(R.string.settings_account_agora_api_key), agoraApiKey).apply();
        this.agoraApiKey = agoraApiKey;
    }

    public String getAgoraApiKey() {
        if(agoraApiKey == null)
            agoraApiKey=  sharedPref.getString(getString(R.string.settings_account_agora_api_key),null);
        return agoraApiKey;
    }

    public void setVideoCallPolicy(String videoCallPolicy) {
        sharedPref.edit().putString(getString(R.string.video_call_pref), videoCallPolicy).apply();
        this.videoCallPolicy = videoCallPolicy;
    }

    public String getVideoCallPolicy() {
        if(videoCallPolicy == null )
            videoCallPolicy=  sharedPref.getString(getString(R.string.video_call_pref),"friends");
        return videoCallPolicy;
    }
}